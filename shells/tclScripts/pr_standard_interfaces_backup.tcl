...
if {[info exists HUNDREDG]} {
    ...
    set_property CONFIG.FREQ_HZ "$actual_freq" [get_bd_ports CLK]
    create_bd_port -dir I -type clk CLK_300
    set_property CONFIG.FREQ_HZ 322265625 [get_bd_ports CLK_300]
} else {
    puts "100G not set!"
    create_bd_port -dir I -type clk CLK
...
if {[info exists HUNDREDG]} {
    set_property -dict [list CONFIG.MAX_BURST_LENGTH {1} CONFIG.NUM_WRITE_OUTSTANDING {2} CONFIG.NUM_READ_OUTSTANDING {2} CONFIG.SUPPORTS_NARROW_BURST {0} CONFIG.ADDR_WIDTH {16} CONFIG.FREQ_HZ "$actual_freq" CONFIG.PROTOCOL {AXI4LITE} CONFIG.HAS_QOS {0} CONFIG.HAS_REGION {0}] [get_bd_intf_ports S_AXI_CONTROL]
    set_property -dict [list CONFIG.NUM_WRITE_OUTSTANDING {2} CONFIG.NUM_READ_OUTSTANDING {2} CONFIG.FREQ_HZ "$actual_freq" CONFIG.DATA_WIDTH {512}] [get_bd_intf_ports S_AXI_MEM_0]
    set_property -dict [list CONFIG.NUM_WRITE_OUTSTANDING {2} CONFIG.NUM_READ_OUTSTANDING {2} CONFIG.FREQ_HZ "$actual_freq" CONFIG.DATA_WIDTH {512}] [get_bd_intf_ports S_AXI_MEM_1]
    ...
    set_property CONFIG.ASSOCIATED_BUSIF {S_AXIS:M_AXIS} [get_bd_ports /CLK_300]
} else {
