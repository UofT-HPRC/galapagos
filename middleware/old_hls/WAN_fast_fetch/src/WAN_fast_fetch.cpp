...
#pragma HLS resource core=AXI4Stream variable = from_dispatch
#pragma HLS resource core=AXI4Stream variable = to_collector
#pragma HLS resource core=AXI4Stream variable = to_slow_fetch
#pragma HLS resource core=AXI4Stream variable = bin1_send
#pragma HLS resource core=AXI4Stream variable = bin2_send
#pragma HLS resource core=AXI4Stream variable = bin3_send
#pragma HLS resource core=AXI4Stream variable = update
#pragma HLS DATA_PACK variable=from_dispatch
#pragma HLS DATA_PACK variable=to_collector
#pragma HLS DATA_PACK variable=to_slow_fetch
#pragma HLS DATA_PACK variable=bin1_send
#pragma HLS DATA_PACK variable=bin2_send
#pragma HLS DATA_PACK variable=bin3_send
#pragma HLS DATA_PACK variable=update
    ...
    else if (!update.empty())
    {
		...
		kern_tag = kernel_request(KERN_BITS-1, KERN_INDEX_BITS);
		kern_index = kernel_request(KERN_INDEX_BITS-1,0);
    ...
    else
    {
		...
		case IPF_STATE_IDLE:
			if (!from_dispatch.empty() && !to_collector.full())
			{
				...
				kern_tag(KERN_TAG_BITS-1,0) = kernel_request(KERN_BITS-1, KERN_INDEX_BITS);
				kern_index(KERN_INDEX_BITS-1,0) = kernel_request(KERN_INDEX_BITS-1,0);
				...
				else if ((bin2.occupied==1)&& (bin2.dest == kernel_request))
				{
				...
				else if ((bin3.occupied==1)&& (bin3.dest == kernel_request))
				{
		...
		case RETRY:
            ...
            slow_fetch_response.data=saved_request;
