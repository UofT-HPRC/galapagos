module rpn_LAN_sequence_number_initializer #(
    ...
    output logic node_to_LAN_seq_num_BRAM_WEN,
    ...
    assign node_to_kernel_ROM_ADDR[BRAM_ADDR_WIDTH-1:NODE_ID_WIDTH] = 0;
    always_comb begin
        node_to_kernel_ROM_ADDR[NODE_ID_WIDTH-1:0] = 0;
        // Case 1: First node to read
        if (r_core_state == STATE_IDLE && i_ap_rst_n != 0)
            node_to_kernel_ROM_ADDR[NODE_ID_WIDTH-1:0] = w_next_node_to_send_req;
        // Case 2: Just transmitted a SEQ NUM REQUEST, and need to transmit another
        else if (r_core_state == STATE_SEND_SEQ_NUM_REQ_TO_LAN 
              && to_nb_LAN_tready == 1
              && r_num_req_to_send_this_round != 0)
        ...
        // Case 3: Waited for a timeout, now we need to send another round of SEQ NUM REQUESTs
            ...
            node_to_kernel_ROM_ADDR[NODE_ID_WIDTH-1:0] = w_first_node_to_resend_req;
        else
            node_to_kernel_ROM_ADDR[NODE_ID_WIDTH-1:0] = 0;
    end
    ...
    assign node_to_LAN_seq_num_BRAM_ADDR[BRAM_ADDR_WIDTH-1:NODE_ID_WIDTH] = 0;
    assign node_to_LAN_seq_num_BRAM_ADDR[NODE_ID_WIDTH-1:0] = from_nb_tdata[LAN_ACK_SENDER_NODE_ID_OFFSET+:LAN_ACK_SENDER_NODE_ID_WIDTH];
    ...
    assign node_to_LAN_seq_num_BRAM_WEN = node_to_LAN_seq_num_BRAM_EN;
