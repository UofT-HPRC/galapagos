...
namespace galapagos{
    ...
    template <class T> 
        class router_net_out: public router_net<T>{
            public:
                router_net_out(std::vector <std::string>  &_kern_info_table, 
                               stream <T> * from_sessions, 
                               bool * _done, 
                               std::mutex * _mutex, 
                               std::string my_address,
...
template <class T> 
    ...
    template <class T> 
    galapagos::router_net_out<T>::router_net_out(std::vector <std::string> &  _kern_info_tables, 
                                                 stream <T> * from_sessions, 
                                                 bool * _done, 
                                                 std::mutex * _mutex, 
                                                 std::string my_address,
            ...
            for(unsigned int i=0; i<this->m_axis_ptr.size(); i++){
                ...
                if(this->m_axis_ptr[i] != nullptr && this->ext_port!= nullptr){
                    ...
                    if(stream_ptr->try_peak(gps)){
                        ...
                        {
#ifdef DEBUG
                            std::cout << " WRITING IN ROUTER NET " << std::endl;
#endif
                            std::vector <T> vect =  stream_ptr->read(&dest, &src);
                            this->ext_port->write((char *)vect.data(), vect.size()*8, dest, src);
                        }
                    }
                }
            }
