...
namespace galapagos
{
    ...
    private:
        ...
        bool printer_galapagos_interface = false;
        short write_id;
        short write_dest;
        short read_id;
        short read_dest;
...
void galapagos::interface<T>::write(galapagos::stream_packet<T> gps)
{
    ...
    if (gps.last)
    {

        {
            ...
            T header;
            header = galapagos::range(sizeof(T) * 8 - 1, 24, 0, 0); // last = 0
            header = galapagos::range(23, 16, header, curr_write.dest);
            header = galapagos::range(15, 8, header, curr_write.id);
            header = galapagos::range(7, 0, header, 0);
...
void galapagos::interface<T>::packet_write(char *data, int size, short dest, short id)
{
    ...
    header = galapagos::range(sizeof(T) * 8 - 1, 24, 0, 0); // last = 0
    header = galapagos::range(23, 16, header, dest);
    header = galapagos::range(15, 8, header, id);
    header = galapagos::range(7, 0, header, size);

    memcpy((char *)curr_write.data, &header, sizeof(T));
    memcpy((char *)curr_write.data + sizeof(T), (char *)data, size * sizeof(T));
    if (printer_galapagos_interface)
    {

        std::cout << std::dec << "src:" << id << "  dest:" << dest << std::endl;
        std::cout << std::hex << std::endl;
        for (int i = 0; i < 4; i = i + 1)
        {
            std::cout << "curr_write.data[" << i << "]:" << (ap_uint<8>)curr_write.data[i] << std::endl;
        }
        for (int i = 64; i < 68; i = i + 1)
        {
            std::cout << "curr_write.data[" << i << "]:" << (ap_uint<8>)curr_write.data[i] << std::endl;
        }
    }
...
char *galapagos::interface<T>::packet_read(size_t *_size, short *_dest, short *_id)
{
    ...
    *_id = curr_read_it->id;
    if (printer_galapagos_interface)
    {
        std::cout << "buff_size: " << std::dec << buff_size << std::endl;
        // std::cout << "read_in_prog_addr: " << read_in_prog_addr << std::endl;
        // std::cout << "sizeof(T): " << sizeof(T) << std::endl;
        std::cout << std::hex;
        for (int i = 0; i < 4; i++)
        {
            std::cout << "curr_read_it->data[" << i << "]:" << (ap_uint<8>)curr_read_it->data[i] << std::endl;
        }
        for (int i = 64; i < 68; i++)
        {
            std::cout << "curr_read_it->data[" << i << "]:" << (ap_uint<8>)curr_read_it->data[i] << std::endl;
